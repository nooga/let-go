...
no custom `*data-readers*`, no JVM host interop on `deftype` (`reify` accepts
`Object` method overrides — `toString` is wired into printing; `hashCode`/`equals`
are accepted and ignored), and no
