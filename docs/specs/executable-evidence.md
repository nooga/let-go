...
A row may instead lead with `|!` (FitNesse SLIM decision-table shape,
spelled `|!` or `| !`). The bang makes the row a slim row: it is kept
whole rather than split into positional cells. The first cell names the
decision table; every remaining cell is a Clojure expression the runner
evaluates. The runner prints a `PASS-CELL`/`FAIL-CELL` line per cell with
the actual value, so a re-rendered page shows where a claim diverged, and
the row passes only when every cell is truthy. A cell that throws is a
failure carrying the reader/compiler message. A `!` anywhere but the first
cell is an ordinary cell.
...
- **`clj-table`** — `include clj-repl` plus a `row => form` rule and a
  `slim-row` rule. A tangled ordinary row is `form \t value \t out [\t
  expect]`; the generator rewrites it into the equivalent `clj-repl`
  statement lines and runs it through the `clj-repl` rules. A table case is
  a repl case. A `|!` slim row keeps its cells and is emitted as a
  `slim-check!` call, which evaluates each cell and annotates it.
...
A slim row additionally emits one line per cell, before the case's own
`ok`/`FAIL` line: `PASS-CELL <id> | <table> | <cell> | actual: <edn>` when
the cell's form evaluated truthy, and the same with `FAIL-CELL` when it was
false or threw. These are the live per-cell annotations the FitNesse
renderer shows; a re-rendered page can consume them to place the diff.
...
`run`'s summary relays only `ok `/`FAIL ` lines, so the per-cell
`PASS-CELL`/`FAIL-CELL` annotations are dropped there.

`render <spec> [out]` runs the spec and re-emits its markdown with slim-table
cells annotated in place, FitNesse-style: a cell whose form is not truthy
gains ` ✗ got <actual>`, and the table name plus passing cells are left
untouched. It writes to `<out>` when given and to stdout otherwise. This is
the "modify the markdown" path; the runner-side `PASS-CELL`/`FAIL-CELL` lines
are the live path for an interactive renderer.
