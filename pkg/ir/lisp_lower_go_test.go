...
func TestLowerGoDotDotPreservesUserSourceSpan(t *testing.T) {
	ensureLoader()

	const sourceName = "dotdot-source.lg"
	const source = "\n\n(let [body (ir.passes.pipeline/expand-all\n              '(.. value (replace \"a\" \"b\")))]\n  (ir.build/build-fn (list 'defn 'chain (vector 'value) body)))"
	c := compiler.NewCompiler(vm.NewConsts(), rt.NS(rt.NameCoreNS))
	c.SetSource(sourceName)
	_, fn, err := c.CompileMultiple(strings.NewReader(source))
	if err != nil {
		t.Fatalf("build dotdot function: %v", err)
	}
	...
	passVarCounter++
	varName := fmt.Sprintf("*dotdot-source-fn-%d*", passVarCounter)
	rt.NS(rt.NameCoreNS).Def(varName, fn)
	infos := runLispExpr(t, fmt.Sprintf(`
	  (vec (mapcat (fn [b]
	                 (mapcat (fn [nid] (ir/source-infos nid %s))
	...
	if !found {
		t.Fatalf("lowered dotdot function lost source span %s:4", sourceName)
	}
}
