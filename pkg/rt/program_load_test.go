...
// TestLoadProgramNamespacesForEntryFrameBracketsCompilingAOT pins the half of
// #796 that the main-chunk bracket alone misses. A program that requires
// another namespace gets an NS table in which MainChunk IS one of the NSOrder
// chunks: the top level — the documented `(when-not *compiling-aot* (-main))`
// guard with it — runs during the namespace load, and
// RunProgramMainChunkForEntryFrame then no-ops by design. The guard has to see
// the var set here too, or the binary runs the entry once on the VM and once
// natively.
func TestLoadProgramNamespacesForEntryFrameBracketsCompilingAOT(t *testing.T) {
	v := CoreNS.LookupLocal(vm.Symbol("*compiling-aot*"))
	if v == nil {
		t.Fatal("*compiling-aot* missing from core")
	}
	...
	const nsName = "entryframensguard"
	nsMu.Lock()
	delete(nsRegistry, nsName)
	nsMu.Unlock()
	_ = DefNSBare(nsName)
	...
	// The multi-namespace shape: MainChunk aliases the last ns chunk, so the
	// replay that follows this load returns early without running anything.
	chunk := mkLogChunk(probe, vm.String("nsload"))
	unit := &bytecode.ExecUnit{
		NSOrder:   []string{nsName},
		NSChunks:  map[string]*vm.CodeChunk{nsName: chunk},
		MainChunk: chunk,
	...
	if err := LoadProgramNamespacesForEntryFrame(unit); err != nil {
		t.Fatalf("LoadProgramNamespacesForEntryFrame: %v", err)
	}

	if seen != vm.TRUE {
		t.Fatalf("*compiling-aot* during namespace load = %v, want true — the guard would run the entry twice", seen)
	}
	if after := v.Root(); after != before {
		t.Fatalf("*compiling-aot* after load = %v, want the prior root %v", after, before)
	}

	// The replay really is a no-op for this shape, so the bracket above is the
	// only thing standing between the guard and a second entry.
	seen = nil
	if err := RunProgramMainChunkForEntryFrame(unit); err != nil {
		t.Fatalf("RunProgramMainChunkForEntryFrame: %v", err)
	}
	if seen != nil {
		t.Fatalf("main-chunk replay ran the aliased chunk again (saw %v)", seen)
	}
}
