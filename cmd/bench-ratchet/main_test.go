...
// merely the run that last moved any metric. A timing-only win still measured
// allocs/bytes — at the same value they already held — so it CONFIRMS them and
// takes the stamp: the current run is the newest evidence that this code state
// still holds that floor. Leaving the old stamp in place would let a stale,
// worse row (from a real prior measurement) outrank a row that was just
// re-verified on newer code, and the gate would attribute newer code's numbers
// to a commit that never measured them.
func TestDeterministicProvenanceSurvivesTimingOnlyImprovement(t *testing.T) {
	...
	// March: faster, identical allocs/bytes — a re-measurement that CONFIRMS
	// the stored floor, not just a timing move.
	...
	if got.AllocsSinceSHA != "marsha" || got.BytesSinceSHA != "marsha" ||
		got.AllocsSinceAt != "2026-03-10T00:00:00Z" || got.BytesSinceAt != "2026-03-10T00:00:00Z" {
		t.Fatalf("deterministic provenance = %q/%q, want marsha/2026-03-10T00:00:00Z for both",
			got.AllocsSinceSHA, got.BytesSinceSHA)
	}

	// February measured this benchmark for real, but March (via profile "a")
	// re-confirmed it more recently — March must win the gate.
	baseline := Baseline{Version: schemaVersion, Machines: map[string]MachineBaseline{
		"a": merged,
		"b": {
	...
	bar := machineIndependentBar(baseline)[benchmark]
	if bar.AllocsPerOp != 10 || bar.BytesPerOp != 100 || bar.AllocsSinceSHA != "marsha" {
		t.Fatalf("bar = %d allocs/%d bytes (%q), want 10/100 (marsha)",
			bar.AllocsPerOp, bar.BytesPerOp, bar.AllocsSinceSHA)
	}
}

// TestDeterministicProvenanceLegacyRowConfirmedByNewerRun is the reviewer's
// exact repro for the row-selection bug fixed alongside the stamping rule
// above: a legacy row (no per-metric stamp at all) that gets re-measured at
// the SAME allocs/bytes by a newer run must win the gate over an older row
// that has its own real stamp — the re-measurement is current evidence for
// this code state, and skipping the restamp made an unstamped-but-correct
// floor lose to a stale-but-stamped one.
func TestDeterministicProvenanceLegacyRowConfirmedByNewerRun(t *testing.T) {
	const benchmark = "pkg.BenchmarkLegacyConfirmed"

	// Profile A: an older machine row with its own real February stamp.
	profileA := MachineBaseline{
		CapturedAt: "2026-02-10T00:00:00Z", CapturedAtSHA: "febsha",
		Benchmarks: map[string]BenchmarkEntry{benchmark: {
			NSPerOp: 100, RatioToAnchor: 10, AllocsPerOp: 15, BytesPerOp: 150,
			BestSinceSHA: "febsha", BestSinceAt: "2026-02-10T00:00:00Z",
			AllocsSinceSHA: "febsha", AllocsSinceAt: "2026-02-10T00:00:00Z",
			BytesSinceSHA: "febsha", BytesSinceAt: "2026-02-10T00:00:00Z",
		}},
	...
	// Profile B: a legacy row — no per-metric stamp at all — holding 10
	// allocs / 100 bytes from before stamping existed.
	legacyB := MachineBaseline{
		CapturedAt: "2026-01-01T00:00:00Z", CapturedAtSHA: "legacysha",
		Benchmarks: map[string]BenchmarkEntry{benchmark: {
			NSPerOp: 200, RatioToAnchor: 20, AllocsPerOp: 10, BytesPerOp: 100,
		}},
	}
	// March run on B: remeasures the SAME 10 allocs / 100 bytes, but faster.
	marchB := MachineBaseline{
		CapturedAt: "2026-03-15T00:00:00Z", CapturedAtSHA: "marsha",
		Benchmarks: map[string]BenchmarkEntry{benchmark: {
			NSPerOp: 90, RatioToAnchor: 9, AllocsPerOp: 10, BytesPerOp: 100,
		}},
	}
	mergedB, _ := ratchetMerge(legacyB, marchB)
	gotB := mergedB.Benchmarks[benchmark]
	if gotB.AllocsPerOp != 10 || gotB.BytesPerOp != 100 {
		t.Fatalf("merged B = %d allocs/%d bytes, want 10/100", gotB.AllocsPerOp, gotB.BytesPerOp)
	}
	if gotB.AllocsSinceSHA != "marsha" || gotB.BytesSinceSHA != "marsha" {
		t.Fatalf("merged B provenance = %q/%q, want marsha/marsha confirming the remeasurement",
			gotB.AllocsSinceSHA, gotB.BytesSinceSHA)
	}
	...
	if bar.AllocsPerOp != 10 || bar.BytesPerOp != 100 {
		t.Fatalf("gate bar = %d allocs/%d bytes, want 10/100 (B, March-confirmed) not 15/150 (A, February)",
			bar.AllocsPerOp, bar.BytesPerOp)
	}
	if bar.AllocsSinceSHA != "marsha" || bar.BytesSinceSHA != "marsha" {
		t.Fatalf("gate bar provenance = %q/%q, want marsha for both", bar.AllocsSinceSHA, bar.BytesSinceSHA)
	}
}

// TestRatchetMergeWorseMeasurementKeepsOldStampPerMetric pins that the ratchet
// only tightens: a run that measures ONE deterministic metric worse than the
// stored bar must not touch that metric's value or stamp, while a metric it
// confirms (equal or better) restamps independently of the other's outcome.
func TestRatchetMergeWorseMeasurementKeepsOldStampPerMetric(t *testing.T) {
	const benchmark = "pkg.BenchmarkMixedMetrics"
	january := MachineBaseline{
		CapturedAt: "2026-01-10T00:00:00Z", CapturedAtSHA: "jansha",
		Benchmarks: map[string]BenchmarkEntry{benchmark: {
	...
	// March: allocs measured WORSE (12 > 10), bytes confirmed equal (100).
	march := MachineBaseline{
		CapturedAt: "2026-03-10T00:00:00Z", CapturedAtSHA: "marsha",
		Benchmarks: map[string]BenchmarkEntry{benchmark: {
			NSPerOp: 90, AllocsPerOp: 12, BytesPerOp: 100,
		}},
	}
	merged, _ := ratchetMerge(january, march)
	got := merged.Benchmarks[benchmark]
	if got.AllocsPerOp != 10 {
		t.Fatalf("merged allocs = %d, want 10 (ratchet keeps the tighter stored value)", got.AllocsPerOp)
	}
	if got.AllocsSinceSHA != "jansha" || got.AllocsSinceAt != "2026-01-10T00:00:00Z" {
		t.Fatalf("allocs provenance = %q/%q, want jansha/2026-01-10T00:00:00Z (worse run must not restamp)",
			got.AllocsSinceSHA, got.AllocsSinceAt)
	}
	if got.BytesPerOp != 100 {
		t.Fatalf("merged bytes = %d, want 100", got.BytesPerOp)
	}
	if got.BytesSinceSHA != "marsha" || got.BytesSinceAt != "2026-03-10T00:00:00Z" {
		t.Fatalf("bytes provenance = %q/%q, want marsha/2026-03-10T00:00:00Z (confirmed run restamps)",
			got.BytesSinceSHA, got.BytesSinceAt)
	}
}
...
// TestForceRebaselineEqualMeasurementConfirmsProvenance is ratchetMerge's
// confirmation rule on the -force path, which is the timing-recapture workflow:
// a forced run that measures the SAME allocs/bytes as a legacy row (no
// per-metric stamp) is current evidence for them and must date them, or the
// row's provenance stays unknown and the gate falls back to an older, worse
// profile. A metric measured worse still keeps its old value and stamp.
func TestForceRebaselineEqualMeasurementConfirmsProvenance(t *testing.T) {
	const benchmark = "pkg.BenchmarkForcedConfirmed"
	const mixed = "pkg.BenchmarkForcedMixed"
	machine := Machine{OS: "darwin", Arch: "arm64", CPUModel: "Apple M3", GoVersion: "go1.26.5"}
	key := perfdata.MachineKey(machine)

	february := MachineBaseline{
		CapturedAt: "2026-02-10T00:00:00Z", CapturedAtSHA: "febsha",
		Benchmarks: map[string]BenchmarkEntry{benchmark: {
			NSPerOp: 100, RatioToAnchor: 10, AllocsPerOp: 15, BytesPerOp: 150,
			BestSinceSHA: "febsha", BestSinceAt: "2026-02-10T00:00:00Z",
	...
	baseline := Baseline{Version: schemaVersion, Machines: map[string]MachineBaseline{
		...
		key: {
			Machine: machine,
			Benchmarks: map[string]BenchmarkEntry{
				// Legacy rows: values from before stamping existed.
				benchmark: {NSPerOp: 200, RatioToAnchor: 20, AllocsPerOp: 10, BytesPerOp: 100},
				mixed: {NSPerOp: 200, RatioToAnchor: 20, AllocsPerOp: 10, BytesPerOp: 100,
					AllocsSinceSHA: "jansha", AllocsSinceAt: "2026-01-10T00:00:00Z",
					BytesSinceSHA: "jansha", BytesSinceAt: "2026-01-10T00:00:00Z"},
			},
		},
	...
	march := MachineBaseline{
		...
		Machine: machine,
		Benchmarks: map[string]BenchmarkEntry{
			benchmark: {NSPerOp: 90, RatioToAnchor: 9, AllocsPerOp: 10, BytesPerOp: 100},
			mixed:     {NSPerOp: 90, RatioToAnchor: 9, AllocsPerOp: 12, BytesPerOp: 100},
		},
	}
	...
	if got.AllocsSinceSHA != "marsha" || got.BytesSinceSHA != "marsha" {
		t.Fatalf("forced equal measurement provenance = %q/%q, want marsha/marsha",
			got.AllocsSinceSHA, got.BytesSinceSHA)
	}
	bar := machineIndependentBar(baseline)[benchmark]
	if bar.AllocsPerOp != 10 || bar.BytesPerOp != 100 {
		t.Fatalf("bar = %d/%d since %s; want March-confirmed 10/100",
			bar.AllocsPerOp, bar.BytesPerOp, bar.AllocsSinceSHA)
	}

	gotMixed := baseline.Machines[key].Benchmarks[mixed]
	if gotMixed.AllocsPerOp != 10 || gotMixed.AllocsSinceSHA != "jansha" {
		t.Fatalf("worse allocs: %d since %q, want the stored 10 since jansha",
			gotMixed.AllocsPerOp, gotMixed.AllocsSinceSHA)
	}
	if gotMixed.BytesPerOp != 100 || gotMixed.BytesSinceSHA != "marsha" {
		t.Fatalf("confirmed bytes: %d since %q, want 100 since marsha",
			gotMixed.BytesPerOp, gotMixed.BytesSinceSHA)
	}
	if len(rejected) != 1 || rejected[0].Name != mixed || rejected[0].Metric != "allocs/op" {
		t.Fatalf("rejected = %+v, want only the allocs regression of %s", rejected, mixed)
	}
}
