...
func specEvidence(t *testing.T, args ...string) ([]byte, error) {
	...
	cmd.Dir = ".."
	// CLJ_ENGINE: the spec-evidence CLI defaults its runner engine to the
	// literal relative "./lg", which does not exist in CI (the harness builds
	// the binary into a temp dir). Point it at the same lgBin driving this
	// test; the engine-selection code already honors this variable.
	cmd.Env = append(os.Environ(), "LG_SOURCE_PATHS=scripts", "CLJ_ENGINE="+lgBin)
