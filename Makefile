...
# Every generated artifact that is COMMITTED. This list and `make generate` are
# the same list: generate.lg produces exactly these, and the gate below cmps
# exactly these. Adding a committed artifact means adding it in both places, and
# the gate fails loudly if a generator stops emitting one.
GENERATED-TRACKED := \
...
#     checkout (an mtime `find -newer` check silently passes after any VCS
#     checkout — in fact the old check-lowered-fresh pointed at a path that no
#     longer exists and had been a silent no-op).
#
#     Regeneration goes through the full `make generate`, not through lgbgen
#     and lgprimgen directly. Driving the narrower path is what let three
#     committed artifacts (op_generated.go, ir_bridge_generated.go,
#     ir/data/generated.lg) go ungated: they are produced only by generate.lg,
#     so a gate that never runs it cannot see them drift.
#
...
# lgbgen's writeBundle refreshes it on every regeneration — without that, a
# check-* target would mutate a tracked file. check-generated-manifest gates the
# digest and is deliberately NOT a prerequisite here: it is a proxy, and a stale
# proxy should not abort the run before the content gate can give the real
# answer. CI still runs it directly, as does TestGeneratedArtifactsAreFresh.
#
...
check-generated: $(GO)
	...
	for f in $(GENERATED-TRACKED); do \
		if cmp -s "$$f" "$$tmp/$$f"; then \
			echo "OK: $$f in lockstep with source."; \
		else \
			echo "ERROR: $$f is stale — the regenerated bytes differ."; \
			rc=1; \
		fi; \
	done; \
	for f in $(GENERATED-TRACKED) pkg/rt/generated.sums; do cp "$$tmp/$$f" "$$f"; done; \
